...
import type { ApiKey, AuditLog, Role, User } from '@/lib/types';
import { useApiMutation, useList } from '@/hooks/useApi';
import { CrudPage } from '@/components/CrudPage';
import { Badge, Button, ConfirmDialog, DataTable, Field, Input, Modal, PageHeader, Pagination, Select } from '@/components/ui';
...
export function UsersPage() {
  return (
    <CrudPage<User>
      title="Usuarios"
      description="Cuentas de acceso y roles"
      entityName="Usuario"
      resource="/users"
      writeRole="ADMIN"
      deleteLabel="Desactivar"
      defaults={{ role: 'VIEWER', isActive: true }}
      ...
      fields={[
        ...
        { name: 'password', label: 'Contraseña', type: 'password', hint: 'Mín. 8 caracteres con letras y números. En edición, déjela vacía para no cambiarla.' },
...
export function ApiKeysPage() {
  ...
  const [form, setForm] = useState({ name: '', role: 'VIEWER', expiresAt: '' });
  ...
  const submit = (e: FormEvent) => {
    e.preventDefault();
    create.mutate({ name: form.name, role: form.role, expiresAt: form.expiresAt ? new Date(form.expiresAt).toISOString() : undefined });
  };
...
export function AuditPage() {
  ...
  const entities = ['Product', 'Category', 'Warehouse', 'Supplier', 'Customer', 'StockMovement', 'PurchaseOrder', 'SalesOrder', 'User', 'ApiKey'];
